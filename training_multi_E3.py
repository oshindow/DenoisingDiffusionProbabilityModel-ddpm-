...
def run(rank, n_gpus):
    ...
    train_dataset = data.CustomAudioDataset('train_E3.txt', tensor_cut=800)
    ...
    def train(epoch, loader):
        ...
        for batch_idx, batch in enumerate(trainloader):
            ...
            diff_loss, _ = model(pro, tim, con, tar, lengths)
            
            # print(pro.shape, tim.shape, lengths)
            with torch.no_grad():
                predict = model.module.inference(pro, tim, con, lengths).squeeze()
            # print(predict.max(), predict.min())
            # print(tar.max(), tar.min())
            mse_loss = mse(predict / 100, tar.transpose(1,2))
