...
class CustomAudioDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # new_length = int(content.shape[-2] * 1.5)
        new_length = target.shape[-2]
        content_resized = F.interpolate(content.transpose(1,2), size=new_length, mode='linear', align_corners=True).transpose(1,2)
        # print(content.shape, content_resized.shape)
        # print(uid, prosody.shape, timbre.shape, target.shape, content_resized.shape)
        ...
        if self.tensor_cut > 0:
            ...
            if target.size()[-2] > self.tensor_cut:
                ...
                # start = int(start / 1.5)
                # dur = int(self.tensor_cut / 1.5)
                content = content[:, start:start+self.tensor_cut,:]
                # print(uid, prosody.shape, timbre.shape, target.shape, content.shape)
